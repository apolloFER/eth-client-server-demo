...
def encode(data):
    ...
    return b''.join((str(len(data)).encode('utf8'), b':', data, b','))
...
class Decoder(object):
    ...
    def __str__(self):
        ...
        if self.data_size is None:
            _bytes = len(self.size_string)
        else:
            _bytes = self.data_out.tell()

        return "<netstring decoder, %i bytes in buffer>" % _bytes
    ...
    def feed(self, data):
        ...
        while self.data_pos < len(data):
            ...
            if self.expecting_terminator:
                ...
                c = data[self.data_pos:self.data_pos + 1]
                self.data_pos += 1
                if c != b',':
                ...
                if c in b"0123456789":
                    ...
                    if not len(self.size_string):
                        raise DecoderError(DecoderError.ILLEGAL_DIGIT_IN_SIZE,
                                           "Illegal digit (%s) in size field" % repr(c))
